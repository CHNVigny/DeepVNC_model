...
class ArithmeticCoderBase:
	...
	def __init__(self, numbits):
		...
		self.full_range = 1 << self.num_state_bits# 1左移self.num_state_bits位 默认是32位。1左移32位在内存里就是1后边32个零。
		# The top bit at width num_state_bits, which is 0100...000.
		self.half_range = self.full_range >> 1  # Non-zero 1后边31个零
		# The second highest bit at width num_state_bits, which is 0010...000. This is zero when num_state_bits=1.
		self.quarter_range = self.half_range >> 1  # Can be zero 1后边30个0
		# Minimum range (high+1-low) during coding (non-trivial), which is 0010...010.
		self.minimum_range = self.quarter_range + 2  # At least 2 1000……0010 （中间28个0）
		...
		# 此算术编码器当前范围的低端。 概念上有无限数量的尾随 0。
		self.low = 0
		# High end of this arithmetic coder's current range. Conceptually has an infinite number of trailing 1s.
		# 此算术编码器当前范围的高端。 概念上有无限数量的尾随 1。实际上等于32个1。
	...
	def update(self, freqs, symbol):
		...
		low = self.low
		high = self.high
		if low >= high or (low & self.state_mask) != low or (high & self.state_mask) != high: #low比high大或者low或者high大于最大范围。
		...
		# Update range
		newlow  = low + symlow  * range // total #整数除法
...
class SimpleFrequencyTable(FrequencyTable):
	...
	def __init__(self, freqs):
		if isinstance(freqs, FrequencyTable):#如果传进来的参数是FrequencyTable类型的对象。
			numsym = freqs.get_symbol_limit()#symbol数量。
			self.frequencies = [freqs.get(i) for i in range(numsym)]#self.frequencies是个列表，记录每个符号出现的频次。
		...
		for freq in self.frequencies: #频次不能是负数。
			if freq < 0:
				raise ValueError("Negative frequency")
		
		# Always equal to the sum of 'frequencies'
		self.total = sum(self.frequencies)#计算一下总的频数。
		...
		# cumulative[i]是从 0（包含）到 i（不包含）的“频率”的总和。
        # 懒惰地初始化。 当它不是 None 时，数据是有效的。
		self.cumulative = None
	
	
	# Returns the number of symbols in this frequency table, which is at least 1.
	# 返回这个频率表里拥有symbol的数量，最少应该有一个。
	def get_symbol_limit(self):
	...
	# Returns the sum of the frequencies of all the symbols strictly
	# below the given symbol value. The returned value is at least 0.
	# 返回严格低于给定符号值的所有符号的频率总和。 返回值至少为 0。所以cumulative[0] = 0
	def get_low(self, symbol):
		self._check_symbol(symbol)
	...
	# cumulative[0]代表symbol值严格低于0的所有符号出现过的总频数。不存在负数的symbol，所以cumulative[0] = 0。
