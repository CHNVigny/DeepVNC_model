...
class ImageCompressor(nn.Module):
    ...
    def forward(self, input_image):
        ...
        recon_sigma = self.priorDecoder(compressed_z)#重建的sigma
        feature_renorm = feature
        if self.training:
            compressed_feature_renorm = feature_renorm + quant_noise_feature#量化
        else:
        ...
        def feature_probs_based_sigma(feature, sigma):
            ...
            probs = gaussian.cdf(feature + 0.5) - gaussian.cdf(feature - 0.5)#cumulative distribution fuction
        ...
        def iclr18_estimate_bits_z(z):
            prob = self.bitEstimator_z(z + 0.5) - self.bitEstimator_z(z - 0.5)#传入的参数是channel
