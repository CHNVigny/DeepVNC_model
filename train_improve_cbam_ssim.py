import os
import argparse
from model_improve_cbam_ssim import *
import torch
...
def train(epoch, global_step):
    ...
    net.train()
    global optimizer
    elapsed, losses, psnrs, bpps, bpp_features, bpp_zs, mse_losses, ms_ssim_losses = [AverageMeter(print_freq) for _ in range(8)]
    # model_time = 0
    # compute_time = 0
    # log_time = 0
    for batch_idx, input in enumerate(train_loader):
        ...
        clipped_recon_image, loss, bpp_feature, bpp_z, bpp, ms_ssim, mse_loss = net(input)
        loss = loss.mean()
        ...
        if (global_step % cal_step) == 0:
            ...
            ms_ssim_losses.update(ms_ssim.item())
        ...
        if (global_step % print_freq) == 0:
            ...
            tb_logger.add_scalar('mse', mse_losses.avg, global_step)
            tb_logger.add_scalar('ms_ssim', ms_ssim_losses.avg, global_step)
            process = global_step / tot_step * 100.0
            log = (' | '.join([
                f'distortion [{distortion:.3f}]',
                f'distribution_loss(bpp) [{distribution_loss:.3f}]',
                ...
                f'MS_SSIM {ms_ssim_losses.val:.5f} ({ms_ssim_losses.avg:.5f})',
            ]))
            logger.info(log)
...
def testKodak(step):
    with torch.no_grad():
        ...
        for batch_idx, input in enumerate(test_loader):
            clipped_recon_image, loss, bpp_feature, bpp_z, bpp, ms_ssim_loss, mse_loss = net(input)
            mse_loss, bpp_feature, bpp_z, bpp = \
                torch.mean(mse_loss), torch.mean(bpp_feature), torch.mean(bpp_z), torch.mean(bpp)

            input_shape = input.shape
            require_shape = input_shape[1::]
            rsp_input_image = input.reshape(require_shape)
            rsp_output_image = clipped_recon_image.reshape(require_shape)

            tb_logger.add_image("ori_image", rsp_input_image, step)
            tb_logger.add_image("out_image", rsp_output_image, step)
...
if __name__ == "__main__":
    ...
    torch.manual_seed(seed=args.seed)#为CPU设置种子用于生成随机数，以使得结果是确定的 #Sets the seed for generating random numbers. Returns a torch.Generator object.
